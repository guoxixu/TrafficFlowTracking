function p = plot_lines_as_patches(xs,ys,varargin)
    % SYNTAX:
    %     plot_lines_as_patches(xs,ys)
    %     plot_lines_as_patches(xs,ys,zs,...)
    %     plot_lines_as_patches(xs,ys,zs,'PropertyName',propertyvalue,...)
    %     p = plot_lines_as_patches(...)
    %
    % PROPERTIES: 
    %     Accepts all parameter-values accepted by PATCH.
    % 
    % DESCRIPTION:
    %     p = plot_lines_as_patches(xs,ys,zs,'PropertyName',propertyvalue,...)
    ...
    % p  = plot_lines_as_patches(xs,ys,zs,'linestyle','--','edgecolor','g',...
    %     'linewidth',3,'edgealpha',0.2);
    %
    %%% Example 2: (Note "hold on" not necessary here!)
    %
    % t = 0:pi/64:4*pi;
    % p(1) = plot_lines_as_patches(t,sin(t),'edgecolor','b','linewidth',2,'edgealpha',0.5);
    % p(2) = plot_lines_as_patches(t,cos(t),'edgecolor','r','linewidth',2,'edgealpha',0.5);
    ...
    %%%         Instead, plot_lines_as_patches plots transparent lines over
    %%%         the image.)
    %
    % img = imread('rice.png');
    % imshow(img)
    ...
    % plot_lines_as_patches(boundaries{ii}(:,2),boundaries{ii}(:,1),...
    ...
    if rem(numel(PVs),2) ~= 0
        ...
        error('plot_lines_as_patches: Parameter-Values must be entered in valid pairs')
    end
